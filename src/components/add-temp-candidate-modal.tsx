...
export function AddTempCandidateModal({ open, onClose, onCreated }: {
  ...
  const emptyFull = () => ({
    ...
    work_ref_1:{referee_name:"",referee_email:"",referee_phone:"",company_name:"",referee_job_title:"",candidate_position:"",employment_start_date:"",employment_end_date:"",reason_for_leaving:""},
    work_ref_2:{referee_name:"",referee_email:"",referee_phone:"",company_name:"",referee_job_title:"",candidate_position:"",employment_start_date:"",employment_end_date:"",reason_for_leaving:""},
  ...
  const saveFromForm=async()=>{
    ...
    const refs=[
      full.work_ref_1.referee_name?{referee_name:full.work_ref_1.referee_name,referee_email:full.work_ref_1.referee_email,referee_phone:full.work_ref_1.referee_phone,company_name:full.work_ref_1.company_name,referee_job_title:full.work_ref_1.referee_job_title,candidate_position:full.work_ref_1.candidate_position,employment_start_date:full.work_ref_1.employment_start_date||null,employment_end_date:full.work_ref_1.employment_end_date||null,reason_for_leaving:full.work_ref_1.reason_for_leaving,candidate_id:cand.id,ref_type:"work",ref_number:1,status:"pending",unique_token:crypto.randomUUID(),short_code:genCode()}:null,
      full.work_ref_2.referee_name?{referee_name:full.work_ref_2.referee_name,referee_email:full.work_ref_2.referee_email,referee_phone:full.work_ref_2.referee_phone,company_name:full.work_ref_2.company_name,referee_job_title:full.work_ref_2.referee_job_title,candidate_position:full.work_ref_2.candidate_position,employment_start_date:full.work_ref_2.employment_start_date||null,employment_end_date:full.work_ref_2.employment_end_date||null,reason_for_leaving:full.work_ref_2.reason_for_leaving,candidate_id:cand.id,ref_type:"work",ref_number:2,status:"pending",unique_token:crypto.randomUUID(),short_code:genCode()}:null,
    ...
    if(refs.length){const{error:refErr}=await supabase.from("references").insert(refs as any[]);if(refErr)console.error("References insert failed:",refErr.message);}
            ...
            {[{title:"Work Referee 1",setR:setRef1,r:full.work_ref_1},{title:"Work Referee 2",setR:setRef2,r:full.work_ref_2}].map(({title,setR,r})=>(
              <div key={title} className="space-y-3"><h3 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{title}</h3>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <F label="Start date"><Input type="date" value={r.employment_start_date} onChange={e=>setR("employment_start_date",e.target.value)} className="h-9"/></F>
                  <F label="End date"><Input type="date" value={r.employment_end_date} onChange={e=>setR("employment_end_date",e.target.value)} className="h-9"/></F>
